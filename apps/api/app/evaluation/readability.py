...
def readability_background(layout, main_visual_path, treatment):
...
def sampled_text_contrast(background: Image.Image, fact: RenderedTextFact) -> float | None:
    box = fact.box
    left, top = max(0, box["x"]), max(0, box["y"])
    right = min(background.width, box["x"] + box["width"])
    bottom = min(background.height, box["y"] + box["height"])
    if right <= left or bottom <= top:
        return None
    sample = background.crop((left, top, right, bottom)).resize((12, 12))
    color = ImageColor.getrgb(fact.color)
    ratios = []
    for y in range(12):
        for x in range(12):
            pixel = sample.getpixel((x, y))
            blended = [
                round(fg * fact.opacity + bg * (1 - fact.opacity))
                for fg, bg in zip(color, pixel, strict=True)
            ]
            foreground, lum = _luminance(blended), _luminance(pixel)
            ratios.append((max(foreground, lum) + 0.05) / (min(foreground, lum) + 0.05))
    ratios.sort()
    return ratios[int((len(ratios) - 1) * 0.1)]
...
def text_readability_checks(
    ...
    *,
    ...
    for fact in text_facts:
        measured = sampled_text_contrast(background, fact)
        if measured is None:
            checks.append(
                VerificationCheck(
                    key=f"readability:{fact.element_id}",
                    label=f"{fact.element_id} 文字可读性",
                    status="unavailable",
                    detail="没有有效文字区域用于对比度采样。",
                )
            )
            continue
        ...
        checks.append(
            VerificationCheck(
                key=f"readability:{fact.element_id}",
                label=f"{fact.element_id} 文字可读性",
                status="passed" if measured >= threshold else "failed",
                after=round(measured, 3),
                detail=(
                    f"包围框背景采样对比度第 10 百分位 {measured:.2f}:1，"
                    f"参考门槛 {threshold:g}:1；不计描边，不证明实际阅读效果。"
                ),
            )
        )
    return checks
